...
function FieldComponent({ id }) {
  ...
  switch (fieldData.crop) {
    case "carrot":
      crop = (
        <div
          className=" absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  cursor-set w-1/2 h-1/2 hover:scale-100 "
  ...
  const imageFiled =
    fieldData.isGrowing || fieldData.isWatered ? (
      ...
      <img
        src={fieldImg}
        alt="field"
        className="w-40 h-auto rounded-3xl max-sm:rounded-xl"
      />
    );
